...
def parse_args():
    ...
    parser_arg.add_argument('--max_epoch', type=int, default=1, help='Maximum epoch number for each round')
    parser_arg.add_argument('--lr', type=float, default=0.0005, help='Learning rate')
    ...
    parser_arg.add_argument('--lambd', type=float, default=0.1, help='Coefficient for components correlation')
    parser_arg.add_argument('--tau', type=float, default=1, help='tau for Eq.23')
...
# saving path
data_path = '/apdcephfs/share_1367250/qinghonglin/hash_datasets/nus-wide/nus-wide.mat'
save_path = '/apdcephfs/private_qinghonglin/hash_DSCH/checkpoints/output/nus-wide/' + version
logfile_dir = '/apdcephfs/private_qinghonglin/hash_DSCH/checkpoints/log/nus-wide/'
...
if args.version == 'debug':
    # train_data, train_data_L = mat['test_data'], mat['test_L'].squeeze()
    train_data, train_data_L = np.array(mat['test_data']).T, np.array(mat['test_L']).T.squeeze()
else:
    # train_data, train_data_L = mat['train_data'], mat['train_L'].squeeze()
    train_data, train_data_L = np.array(mat['train_data']).T, np.array(mat['train_L']).T.squeeze()
...
with graph.as_default(), tf.device('/gpu:' + str(num_gpus)):
    ...
    tau_s = tf.compat.v1.placeholder(tf.float32, shape=[])
        ...
        with tf.compat.v1.device('/gpu:' + str(num_gpus)):
            with tf.name_scope('tower_0') as scope:
                H_code, sim1, sim2, sim3 = Inference(input, train_mode=train_mode, codelen=codelen,
                                                               components_F=MU_1_s, components_C=MU_2_s, tau=tau_s )
...
for r in range(round):
    ...
    while epoch < max_epoch:
        ...
        MU_1_ = np.sign(gmm.means_)  # Representation of fine-grained components: \mu^1_{j} of Eq (10)
        ...
        kmean.fit(gmm.means_)

        P_21_ = one_hot_label_db(
            kmean.labels_)  # Assignment of fine-grained components to coarse-grained: p(C^2|C^1) of Eq (12)
        MU_2_ = np.sign(kmean.cluster_centers_)  # Representation of coarse-grained components: \mu^2_{j} of Eq (13)
        ...
        for i in tqdm.tqdm(range(total_batch)):
            ...
            loss_, loss_ic_, loss_cc_f_, loss_cc_c_, _ = \
                sess.run([loss, loss_ic, loss_cc_f, loss_cc_c, train_g],
                         feed_dict={input: next_batch, lambd_s: lambd, tau_s: tau,
                                    S_s: S_batch,
    ...
    logger.info('Round=' + str(r) + ' codelen=' + str(codelen) + ' map:{:.4f}, map_1000:{:.4f}, map_5000:{:.4f}'.
                format(map_all, map_1000, map_5000))
